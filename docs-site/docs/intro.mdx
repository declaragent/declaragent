...
<div className="dledger">
  ...
  <div className="dledger__row"><span><strong>Independent agents</strong> — own session + secrets; optional delegation via Kafka/NATS RPC (SQS/AMQP/MQTT as library transports)</span><span className="dgrade dgrade--ok">✓ single-machine</span><span className="dgrade dgrade--warn">◐ enterprise</span></div>
  <div className="dledger__row"><span><strong>Tools + MCP</strong> — 7 built-ins (+ SendMessage with channels), MCP (stdio verb; HTTP/SSE via config), npm plugins</span><span className="dgrade dgrade--ok">✓ single-machine</span><span className="dgrade dgrade--warn">◐ enterprise</span></div>
