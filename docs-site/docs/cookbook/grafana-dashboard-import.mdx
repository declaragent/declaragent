...
groups:
  - name: declaragent
    rules:
      ...
      - alert: DeclaragentSourceDlqGrowing
        # note: this is the per-source DLQ counter; dispatch-DLQ depth has
        # no Prometheus series today — inspect it via `declaragent dlq list`
        expr: rate(source_messages_dlq[10m]) > 0
        for: 10m
        labels: { severity: warning }
...
Metrics stay Prometheus-pull-only — the OTLP endpoint exports **spans**, not
metrics (and needs the optional `@opentelemetry/*` peer deps installed). See
[Trace a request in Grafana](/cookbook/grafana-tracing) for the span recipe that
pairs with this dashboard.
