...
- **Back-pressure** — when the exporter falls behind, the daemon slows the ingress side instead of silently dropping events (`declaragent_audit_backpressure_paused_total`).
- **Adaptive batch interval** — the loop widens `intervalMs` under steady load and collapses it back when the queue drains (`declaragent_audit_batch_interval_ms`).
...
Supported auth modes: `apiKey`, `basic` (username + password), `bearer` (the elastic auth union in `packages/core/src/agents/load-agent.ts`).
...
| `declaragent_audit_export_last_seq` flat while audit writes continue | Exporter cursor drifting; check SIEM health (there is no chain-head gauge to diff against yet) |
...
audit:
  export:
    ...
    hecUrl: …
    token: env:SPLUNK_HEC_TOKEN
    index: "declaragent_audit"   # a plain string — no per-tenant templating today
```

Every exported record carries its `tenantId` field — route per-tenant at SIEM
ingest (props/transforms in Splunk, ingest pipeline in Elastic). Combine with the
[Multi-tenant starter](/cookbook/multi-tenant-starter) for full isolation.
