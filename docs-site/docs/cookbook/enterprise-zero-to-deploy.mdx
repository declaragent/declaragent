...
The builder toolkit is gated behind `DECLARAGENT_BUILDER=on` (strictly lowercase
`on`) — a bare `declaragent` opens the plain REPL with **no** `Declara*` tools.
Every REPL launch in this walkthrough assumes the flag is set.
...
Both the **builder tool** `DeclaraAddMCP` and the CLI verb `declaragent mcp add` support **stdio transport only** today (`mcp add <name> --command <cmd> [--args ...]` — there are no `--transport`/`--url` flags yet). All MCP servers live in **user-global** `~/.declaragent/mcp-servers.json`, shared across every agent on the machine.
...
Tools from this server appear as `mcp__fs-orders__<tool>` after the REPL restarts. For a remote MCP server, hand-edit `~/.declaragent/mcp-servers.json` today — the runtime supports `http`, `sse`, and `http-streamable` transports, but the `mcp add` verb's remote-transport flags haven't shipped yet:

```json
{
  "version": 1,
  "servers": [
    { "name": "gh", "transport": { "type": "http", "url": "https://mcp.github.com" } }
  ]
}
```
...
Builder:
  ...
  Proposal pr_06 — Slack OAuth playbook + channel registration
    1. [authPlaybook] slack       # the builder's DeclaraAuthPlaybook tool —
       preview:                   # there is no `declaragent auth playbook` shell verb
...
Emits Mermaid (`renderMermaid` in `fleet-graph-cli.ts` — node declarations plus `-->|capability (transport)|` edges):

```mermaid
graph LR
  n_concierge["agent://concierge (client)"]
  n_pr_reviewer["agent://pr-reviewer (server)"]
  n_concierge -->|review-pr (memory)| n_pr_reviewer
```
  ...
  Proposal pr_07 — Run fleet locally (foreground)
    1. [runCommand] declaragent fleet run
       preview:
         ...
         # note: the :9464 /metrics listener is an `up -d` daemon feature —
         # `fleet run` keeps in-memory worker counters only
...
# the webhook listener binds port 7777 by default — match the `port:` in
# concierge's event-sources.yaml if you set one explicitly
curl -sX POST http://localhost:7777/webhook/concierge \
...
Step 5 exits non-zero if any peer lacks an `auth:` block (`fleet-audit-rpc-cli.ts`). If gaps exist, the builder follows up with `--suggest-enable` diffs (peer auth providers are `oidc`, `oauth2-client`, and `hmac` — shared-secret via `keyId`/`secretRef`, the zero-infra default — `peers-loader.ts`). Go deeper: [SIEM audit export](/cookbook/siem-audit-export), [zero-trust RPC migration](/cookbook/zero-trust-rpc-migration).
...
- **Remote MCP servers.** Both `DeclaraAddMCP` and `declaragent mcp add` are stdio-only; hand-edit `~/.declaragent/mcp-servers.json` for `http`/`sse`/`http-streamable` servers (the runtime supports them — the add verb doesn't yet).
