...
`fleet-starter` is a whole-fleet reference (two agents under `agents/`), so it's
copied from the repo rather than unpacked by `declaragent init --template` (which
handles single-agent templates only):

```bash
npx degit declaragent/declaragent/templates/fleet-starter acme-fleet
# or: git clone --depth 1 https://github.com/declaragent/declaragent \
#     && cp -r declaragent/templates/fleet-starter acme-fleet
cd acme-fleet
bun install
```
