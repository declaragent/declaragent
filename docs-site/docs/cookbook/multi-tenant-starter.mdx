...
declaragent down                          # when finished
```

Cross-tenant boundary-violation test:

```bash
declaragent audit query --tenant beta-tenant --kind tenant_boundary_violation
```
...
- **Per-tenant bus strategy.** Each tenant has its own in-memory bus; no message can cross the boundary without a `tenant_boundary_violation` audit record.
