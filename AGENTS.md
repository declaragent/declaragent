...
| `/healthz`+`/readyz` auth-exempt routes; renderers run foreground `up`; full agent-dir ConfigMap; `/readyz` readiness probe; control-plane **safe-subset bind**; `Dockerfile` | ✅ **live-verified** | `control-plane-server.ts`; `k8s-renderer.ts` (ConfigMap embed + readyProbe); `up-cli.ts` `resolveBindAddress` safe-subset; `Dockerfile`. **Rendered manifest deployed to minikube → pod Ready (1/1, 0 restarts).** Safe-subset bind found+fixed there (probes 403'd without it). Helm parity + in-container build = follow-ups. |
