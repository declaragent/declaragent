...
| 24 | [ ] SQS / AMQP / MQTT RPC transport factories (same pattern as Kafka + NATS) | Transport | 2 wk | Partial — SQS shipped 0.7.3, AMQP + MQTT deferred to 0.7.4 Sprint 4 | `agent-b/transport-sprint-3-item-24a-sqs` — `packages/plugin-agent-rpc/src/sqs-transport.ts` + 22-case unit suite (`sqs-transport.test.ts`) covering standard/FIFO publish, at-least-once subscribe, handler-throw leaves message for native SQS DLQ redrive, three decode-fail policies (`delete` / `leave` / `send-dlq`), back-pressure via `maxInFlight`, credential-chain default + LocalStack endpoint. AMQP + MQTT tracked for Sprint 4. |
