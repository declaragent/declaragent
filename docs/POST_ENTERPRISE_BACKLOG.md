...
| 50 | [x] Slice 3 of `CONTROL_PLANE_PLAN.md` — CLI fan-out across hosts (`declaragent fleet ps / events / dlq / logs` call each host's `/status` `/events` `/dlq` `/logs`) | Platform | 2 wk | Shipped (0.7.4) — branch `agent-d/control-plane-sprint-4-item-50` | New `fleet.yaml#hosts[]` block (`FleetHost` schema in core) + `CrossHostControlPlaneClient` (`packages/cli/src/cross-host-control-plane-client.ts`, fetch-based, `env:/file:/literal` bearer, per-call timeout, `{ok, err}` discriminated-union fan-out) + all four verbs wired via `fleet-cross-host-cli.ts`. `--host <name>` filters to one host; `--json` is deterministic. One bad host yields a tagged-failure trailer + exit 1 but never tanks survivors. `fleet logs -f` (live multi-host SSE multiplex) deferred to Slice 6 — 0.7.4 ships snapshot-only logs. 24 new tests (5 schema / 13 client / 6 CLI-verb). |
